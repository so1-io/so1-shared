 ...
 */
export function transformYamlBrandKit(yamlData: Record<string, unknown>): Record<string, unknown> {
  const result: Record<string, unknown> = {};

  for (const [key, value] of Object.entries(yamlData)) {
    const mappedKey = BRAND_KIT_FIELD_MAP[key as keyof typeof BRAND_KIT_FIELD_MAP] ?? key;

    // Recursively transform nested objects
    if (value && typeof value === "object" && !Array.isArray(value)) {
      result[mappedKey] = transformYamlBrandKit(value as Record<string, unknown>);
    } else {
